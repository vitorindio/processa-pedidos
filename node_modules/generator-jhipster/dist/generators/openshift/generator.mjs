...
import { applicationTypes, databaseTypes, messageBrokerTypes, monitoringTypes, openshiftPlatformTypes, searchEngineTypes, serviceDiscoveryTypes, } from '../../jdl/jhipster/index.mjs';
    ...
    get initializing() {
        return {
            sayHello() {
                ...
                this.log.log(chalk.white(`Files will be generated in folder: ${chalk.yellow(this.destinationRoot())} or in the root directory path that you select in the subsequent step`));
            },
            ...super.initializing,
            checkOpenShift: runAsync(function () {
                ...
                shelljs.exec('oc version', { silent: true }, (code, stdout, stderr) => {
                    if (stderr) {
                        this.log.warn('oc 1.3 or later is not installed on your computer.\n' +
            ...
            displayOpenshiftDeploymentProcedure() {
                ...
                for (let i = 0, regIndex = 0; i < this.appsFolders.length; i++) {
                    ...
                    if (app.searchEngine === ELASTICSEARCH) {
                        this.log.verboseInfo(`  ${chalk.cyan(`oc process -f ${this.directoryPath}ocp/${appName}/${appName}-elasticsearch.yml | oc apply -f -`)}`);
                    }
                    if (app.serviceDiscoveryType !== NO_SERVICE_DISCOVERY && regIndex++ === 0) {
                        this.log.verboseInfo(`  ${chalk.cyan(`oc process -f ${this.directoryPath}ocp/registry/application-configmap.yml | oc apply -f -`)}`);
                        if (app.serviceDiscoveryType === EUREKA) {
                            this.log.verboseInfo(`  ${chalk.cyan(`oc process -f ${this.directoryPath}ocp/registry/jhipster-registry.yml | oc apply -f -`)}`);
                        }
                        else {
                    ...
                    if (app.prodDatabaseType !== NO_DATABASE) {
                        this.log.verboseInfo(`  ${chalk.cyan(`oc process -f ${this.directoryPath}ocp/${appName}/${appName}-${app.prodDatabaseType}.yml | oc apply -f -`)}`);
                    }
                    this.log.verboseInfo(`  ${chalk.cyan(`oc process -f ${this.directoryPath}ocp/${appName}/${appName}-deployment.yml | oc apply -f -`)}`);
                }

    ...
    return {
        callOpenApiGenerator() {
            ...
            Object.keys(this.clientsToGenerate).forEach(cliName => {
                ...
                openApiCmd.push(`-g ${openApiGeneratorName}`, `-i ${inputSpec}`, `-p library=${openApiGeneratorLibrary}`, `-p apiPackage=${cliPackage}.api`, `-p modelPackage=${cliPackage}.model`, `-p basePackage=${this.packageName}.client`, `-p configPackage=${cliPackage}`, `-p title=${_.camelCase(cliName)}`, `-p artifactId=${_.camelCase(cliName)}`);
        ...
        addJacksonDataBindNullable() {
            if (!this.enableSwaggerCodegen) {
                ...
                if (this.buildTool === MAVEN) {
                    ...
                    this.editFile('gradle.properties', addGradlePropertyCallback({ property: 'jacksonDatabindNullableVersion', value: jacksonDatabindNullableVersion }));
                    // TODO addGradleDependencyCallback is an internal api, switch to source api when converted to BaseApplicationGenerator
                    this.editFile('build.gradle', addGradleDependencyCallback({

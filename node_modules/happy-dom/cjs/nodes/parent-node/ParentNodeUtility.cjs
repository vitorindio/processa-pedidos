...
class ParentNodeUtility {
    ...
    static append(parentNode, ...nodes) {
        for (const node of nodes) {
            if (typeof node === 'string') {
                XMLParser_js_1.default.parse(parentNode.ownerDocument, node, { rootNode: parentNode });
            }
            else {
    ...
    static prepend(parentNode, ...nodes) {
        ...
        for (const node of nodes) {
            if (typeof node === 'string') {
                const newChildNodes = XMLParser_js_1.default.parse(parentNode.ownerDocument, node).childNodes.slice();

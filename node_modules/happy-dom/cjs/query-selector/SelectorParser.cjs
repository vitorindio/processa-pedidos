...
class SelectorParser {
    ...
    static getSelectorGroups(selector) {
        if (selector === '*') {
            return [[new SelectorItem_js_1.default({ tagName: '*' })]];
        }
        const simpleMatch = selector.match(SIMPLE_SELECTOR_REGEXP);
        if (simpleMatch) {
            if (simpleMatch[1]) {
                return [[new SelectorItem_js_1.default({ tagName: selector.toUpperCase() })]];
            }
            else if (simpleMatch[2]) {
                return [[new SelectorItem_js_1.default({ classNames: selector.replace('.', '').split('.') })]];
            }
            else if (simpleMatch[3]) {
                return [[new SelectorItem_js_1.default({ id: selector.replace('#', '') })]];
            }
        }
                ...
                if (match[1]) {
                    ...
                    switch (match[17].trim()) {
                        case ',':
                            ...
                            break;
                        case '>':
                            currentSelectorItem = new SelectorItem_js_1.default({ combinator: SelectorCombinatorEnum_js_1.default.child });
